...
export const requestResetPassword = (email) => async (dispatch) => {
  ...
  try {
    await axios.post(`${backendUrl}/api/auth/resetPassword/request`, { email });
...
export const confirmResetPassword = (token, newPassword) => async (dispatch) => {
  ...
  try {
    await axios.post(`${backendUrl}/api/auth/resetPassword/confirm`, { token, newPassword });
...
export const getUserById = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(`${backendUrl}/api/auth/${id}`);
    const user = response.data;
